...
For the initial MedliNER corpus, use one entity label: `DiseaseOrPhenotypicFeature`. DAKP prompts GLiNER with separate disease/phenotype types but merges them downstream, so prompting one merged condition label keeps the annotation policy, the gold benchmark, and the GLiNER pre-labeling prompts compatible with DAKP's end-to-end behavior. A third `drug` label was specified in the original version of this plan and has since been dropped from the schema; medications get no span (`docs/ANNOTATION_GUIDE.md` rule 4). We can separately evaluate a type-agnostic boundary metric later.
...
3. Convert the validated records into GLiNER's training representation: model-tokenized text plus `(start_token, end_token_inclusive, label)` tuples for `DiseaseOrPhenotypicFeature`; keep original Label Studio character offsets for audit/evaluation and test the conversion against GLiNER's actual `words_splitter`.
...
- `configs/label_studio_ner.xml` — Label Studio browser configuration for `DiseaseOrPhenotypicFeature` spans and indication/contraindication context display.
...
- [x] Finalize the single-label schema (`DiseaseOrPhenotypicFeature`, merged from the original `disease`/`phenotype` pair) and annotation guide, including maximal-span, qualifier, hedge, population, medication-exclusion, and no-entity policies.
- [x] Define the Label Studio Community Edition setup: local self-hosted install (pip or Docker), NER labeling configuration with `DiseaseOrPhenotypicFeature`, visible indication/contraindication task metadata, and JSON/JSONL export procedure. Document the annotator workflow as: open task → click-drag/highlight the condition phrase → choose its label → submit; no manual offset counting. Keep Label Studio outside the core training environment if desired.
