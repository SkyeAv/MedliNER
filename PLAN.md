...
3. Convert the validated records into GLiNER's training representation: model-tokenized text plus `(start_token, end_token_inclusive, label)` tuples for `disease`, `phenotype`, and `drug`; keep original Label Studio character offsets for audit/evaluation and test the conversion against GLiNER's actual `words_splitter`.
...
- `configs/label_studio_ner.xml` — Label Studio browser configuration for `disease`/`phenotype`/`drug` spans and indication/contraindication context display.
...
- [x] Define how candidate tasks are created outside this repository, including DAKP-derived indication/contraindication text and optional pre-annotations; reviewed exports are the only training input.
...
- [x] Design a minimal local Dagster asset graph: Label Studio export → validated/normalized dataset → frozen splits → training run → evaluation report → export bundle. Start with assets, materialization, metadata, and asset checks; do not add schedules/sensors or deployment infrastructure initially.
